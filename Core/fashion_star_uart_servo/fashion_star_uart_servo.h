...
// 直接读取寄存器实现 GetTick，彻底解决 "unresolved external" 链接错误
static inline uint32_t FSUS_GetTick(void) {
    /* TC264 的 STM0 频率通常为 100MHz (即每毫秒 100,000 次计数)
     * 如果你的系统时钟不是 100MHz，请修改 100000 这个常数
     * 公式: 系统频率 / 1000 */
    return (uint32_t)(MODULE_STM0.TIM0.U / 100000);
}
